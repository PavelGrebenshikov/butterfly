...
class TestProductsFilterForm:
    ...
    def test_filtred_products(self, created_model_objects, min_price, max_price):
        ...
        assert (
            form.get_filtered_products()[0].name == "Second"
            and form.get_filtered_products()[1].name == "Third"
        )


class TestProductsSortForm:
    @pytest.mark.parametrize(
        "test_sort, price, product_name",
        [
            ("sort", "price_desc", "Third"),
            ("sort", "price_asc", "First"),
        ],
    )
    def test_sorted_products(
        self, created_model_objects, test_sort, price, product_name
    ):
